...
const App: React.FC = () => {
  ...
  const parseInsights = (text: string) => {
    ...
    // 2. [2단계-핵심] URL 및 참조 문구 강력 제거 (순서 중요)
    // (참조: ...) 괄호로 묶인 모든 내용을 먼저 삭제
    processed = processed.replace(/\(참조:[\s\S]*?\)/gi, '');
    // 괄호 없이 '참조:', 'Source:', '출처:' 뒤에 나오는 URL 패턴 삭제
    processed = processed.replace(/(참조|출처|Source):\s*https?:\/\/[^\s\n]+/gi, '');
    // 본문에 섞여 있는 나머지 모든 http/https URL 삭제
    processed = processed.replace(/https?:\/\/[^\s\)]+/g, '');
    
    // 3. [3단계] 번호 뭉침 방지
    // 마침표 뒤에 바로 숫자가 붙으면 줄바꿈 강제 삽입
    processed = processed.replace(/([.!?])\s*(\d+\.)/g, '$1\n\n$2');

    // 4. [4단계] 불필요한 마크다운 및 공백 제거
    processed = processed
      .replace(/\*\*/g, '') // 볼드체 제거
      .replace(/\[.*?\]/g, '') // [1] 같은 인용 번호 제거
      .trim();
        ...
        <header className="sticky top-0 z-40 bg-transparent px-12 py-10">
          <div className="max-w-6xl mx-auto space-y-6">
            <form onSubmit={handleSearch} className="relative group">
              ...
              {/* 입력창 - pl-24를 적용하여 텍스트와 아이콘 간격 확보 */}
              <input 
                type="text" 
